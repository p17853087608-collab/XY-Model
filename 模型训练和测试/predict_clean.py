...
class TemperatureScaledModel(nn.Module):
    """温度缩放模型包装器 - 提高预测置信度"""
    def __init__(self, base_model, temperature=0.7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 应用温度缩放：T < 1 增强置信度，T > 1 降低置信度
        scaled_logits = logits / self.temperature
        return scaled_logits

def load_model(model_path=r'best.pth', num_classes=2, use_temperature_scaling=True):
    """加载用户定义的ResNet模型及参数"""
    # 创建用户模型实例
    base_model = ResNet1(block=Residual, num_classes=num_classes, in_channels=1)
    ...
    if use_temperature_scaling:
        model = TemperatureScaledModel(base_model, temperature=0.85)  # 更温和的温度缩放
        print(f"启用温度缩放 (T=0.85) 用于概率分布校准")
    else:
        model = base_model
        print("使用标准模型（无温度缩放）")
...
def predict_folder(model, folder_path, output_folder="save_data", use_tta=True, use_confidence_boost=False):
    """预测单个文件夹中的所有图片
    ...
    # 定义扩展的TTA（Test Time Augmentation）变换 - 物理不变性增强
    tta_transforms = [
        # 原始图像
        base_preprocess,
        ...
        transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.RandomAffine(degrees=0, scale=(0.95, 0.95)),  # 缩小5%
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5], std=[0.5])
        ]),
        ...
        transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.RandomAffine(degrees=0, scale=(1.05, 1.05)),  # 放大5%
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5], std=[0.5])
        ]),
    ]
...
def predict_all_folders(base_path=r'32x32_data_set\test', output_folder="save_data", use_tta=True):
    """预测所有文件夹中的图片
    ...
    # 加载模型
    print("正在加载模型...")
    model = load_model(use_temperature_scaling=use_temperature_scaling)  # 根据参数决定是否启用温度缩放
    ...
    for folder in tqdm(folders, desc="处理文件夹"):
        ...
        results, stats = predict_folder(model, folder_path, output_folder, use_tta, use_confidence_boost)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--base_path', type=str, default=r'32x32_data_set\test', help='测试数据路径')
    parser.add_argument('--output', type=str, default="save_data", help='输出文件夹')
    
    args = parser.parse_args()
    
    use_tta = not args.no_tta
    use_confidence_boost = False  # 默认禁用置信度增强
    use_temperature_scaling = False  # 同时禁用温度缩放
    
    print(f"开始预测，TTA模式: {'启用' if use_tta else '禁用'}")
    print(f"置信度增强: {'启用' if use_confidence_boost else '禁用'}")
    print(f"温度缩放: {'启用' if use_temperature_scaling else '禁用'}")
