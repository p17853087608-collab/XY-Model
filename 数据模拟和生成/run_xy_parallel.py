...
def main():
    ...
    # 温度设置 - 三种模式选择一种：
    TEMPERATURE_MODE = "custom"  # "range", "custom", "critical"
    
    # 模式1: 温度范围
    TEMPERATURE_RANGE = (0.85, 1.45)  # (最低温度, 最高温度)
    ...
    # 模拟参数
    LATTICE_SIZE = 32                # 晶格大小 (16, 32, 64)
    EQUILIBRIUM_STEPS = 2000         # 平衡步数
    MEASUREMENT_STEPS = 8000          # 测量步数
    INTERACTION_CONSTANT = 1.0       # 相互作用常数
    
    # 并行计算参数
    NUM_PROCESSES = 4                 # 并行进程数 (建议 <= CPU核心数)
    RANDOM_SEED = 42                  # 随机种子
