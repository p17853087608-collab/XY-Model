...
def extract_temperature_from_filename(filename):
    """
    从文件名中提取温度值
    文件名格式: spin_vis_spin_config_t_3.900_raw.npy.png
    """
    # 使用正则表达式匹配温度值
...
def extract_random_number_from_path(filepath):
    ...
    路径格式: .../simulation_results_20251217_125520_8509/...
    """
    # 分割路径，找到包含simulation_results的部分
    path_parts = filepath.split(os.sep)
    
    for part in path_parts:
        if part.startswith('simulation_results_'):
            # 提取时间戳后的随机数部分
            # 格式: simulation_results_YYYYMMDD_HHMMSS_XXXX
            parts = part.split('_')
            if len(parts) >= 4:
...
def determine_phase_by_temperature_start(temperature):
    """
    根据温度起始点判断相类型
    温度起始点大于1.0: Amorphous phase
    温度起始点小于等于1.0: Ordered phase
    """
    if temperature > 1.0:
